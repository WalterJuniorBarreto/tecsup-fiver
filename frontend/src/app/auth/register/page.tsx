...
export default function RegisterPage() {
  ...
  const handleGithubLogin = () => {
    ...
    console.log("Rol guardado para GitHub:", role);
    window.location.href = `https://github.com/login/oauth/authorize?client_id=${clientId}&scope=user:email&state=${role}`;
  };

    const { isGoogleLoading, googleError, handleGoogleSuccess, handleGoogleError } = useGoogleAuth(role as 'CLIENT' | 'FREELANCER');
    ...
    <div className="min-h-screen bg-black text-white flex flex-col items-center justify-center px-4 py-8">
      ...
      <div className="w-full max-w-md">
       ...
       <form onSubmit={handleSubmit} noValidate className="space-y-4">
        ...
        <div className="grid grid-cols-2 gap-3 mb-2">
          ...
          <div className="relative w-full">
            ...
            <div className="w-full h-[40px] flex items-center justify-center [&>div]:w-full">
              <GoogleLogin
       ...
       < p className="text-center mt-6 text-xs text-zinc-500">
          ¿Ya tienes una cuenta? <Link href="/auth/login" className="text-emerald-400 font-bold hover:underline">Iniciar sesión</Link>
        </p>
    </div>
    </div>
  );
