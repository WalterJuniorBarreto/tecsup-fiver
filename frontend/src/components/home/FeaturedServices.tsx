...
export default function FeaturedServices({ activeCategory, searchTerm, onReset }: FeaturedServicesProps) {
  ...
  return (
    <section className="theme-page py-32 px-10">
        ...
        <div className="flex flex-col md:flex-row justify-between items-end mb-16 gap-6">
          <div>
            <div className="flex items-center gap-3 mb-4">
              ...
              <h2 className="theme-text text-4xl font-black italic uppercase tracking-tighter">Servicios destacados</h2>
            </div>
            <p className="theme-muted text-lg font-medium">Los mejores freelancers listos para trabajar contigo</p>
          </div>
          <button 
            onClick={onReset}
            className="theme-secondary theme-border px-8 py-3 rounded-2xl border hover:bg-[var(--bg-soft)] hover:text-[var(--text-primary)] transition-all font-bold text-sm uppercase tracking-widest"
          >
            Ver todos
          </button>
        </div>
        ...
        {filteredServices.length > 0 ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {filteredServices.map((s) => (
              <div key={s.id} className="theme-card rounded-[2.5rem] overflow-hidden border group hover:border-[#00e676]/30 transition-all flex flex-col hover:shadow-[0_0_50px_-12px_rgba(0,230,118,0.15)]">
                ...
                <div className="p-8 flex flex-col flex-grow">
                  <div className="flex items-center gap-3 mb-6">
                    <img src={s.avatar} alt={s.author} className="w-8 h-8 rounded-full ring-2 ring-[var(--border-strong)]" />
                    <span className="theme-secondary text-xs font-black uppercase tracking-widest">{s.author}</span>
                  </div>

                  <h3 className="theme-text text-xl font-bold leading-tight group-hover:text-[#00e676] transition-colors mb-8 min-h-[3.5rem]">
                    {s.title}
                  </h3>

                  <div className="theme-border mt-auto pt-6 border-t flex justify-between items-center">
                    <div className="flex items-center gap-2">
                      <Star className="w-4 h-4 text-[#00e676] fill-[#00e676]" />
                      <span className="theme-text font-black text-sm">{s.rating.toFixed(1)}</span>
                      <span className="theme-muted text-xs font-bold">({s.reviews})</span>
                    </div>
                    <div className="text-right">
                      <p className="theme-muted text-[10px] font-black uppercase tracking-tighter">Desde</p>
                      <p className="theme-text text-2xl font-black">S/ {s.price}</p>
                    </div>
                  </div>
                </div>
              </div>
          ...
          <div className="theme-border py-20 text-center border-2 border-dashed rounded-[3rem]">
            <SearchX size={48} className="theme-muted mx-auto mb-4" />
            <h3 className="theme-secondary text-xl font-bold">No encontramos servicios que coincidan.</h3>
